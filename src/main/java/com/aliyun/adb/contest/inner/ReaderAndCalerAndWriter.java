...
public class ReaderAndCalerAndWriter implements AnalyticDB {
    ...
    public static final int readerNum = 5;
    public static final int calNum = 5;
    public static final int writeNum = 5;
    ...
    public static ByteBufferCache[][][] byteBufferCaches;
    ...
    public static LongAdder writeLA = new LongAdder();
    /**
     *
    ...
    private void initBufferAndChannelForEachBucket() throws FileNotFoundException {
        fileChannels = new FileChannel[colLen][bucketsNum][writeNum];
        byteBufferCaches = new ByteBufferCache[calNum][colLen][bucketsNum];
        for (int i = 0; i < colLen; i++) {
            for (int j = 0; j < bucketsNum; j++) {
        ...
        for (int i = 0; i < calNum; i++) {
            for (int j = 0; j < colLen; j++) {
                for (int k = 0; k < bucketsNum; k++) {
                    byteBufferCaches[i][j][k] = new ByteBufferCache(j, k);
                }
            }
        }
    }
    ...
    private void readAndWrite(File dataFile) throws IOException, InterruptedException {
        ...
        for (int i = 0; i < readerNum; i++) {
            ...
            for (int j = 0; j < calNum / readerNum; j++) {
                ...
                caler[i * calNum / readerNum + j] = new Thread(new CalThread(i * calNum / readerNum + j, readAndCalCaches[j],
                        calAndWriteCache, byteBufferCaches[i * calNum / readerNum + j]));
            }
        ...
        System.out.println("writer blocking times: " + writeLA.sum());
    }
    ...
    private void loadInMemroy(File dataFile) throws IOException, InterruptedException {
        ...
        initBufferAndChannelForEachBucket();
